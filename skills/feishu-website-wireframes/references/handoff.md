...
每段补一句主要结论，并说明主信息、证据和补充的关系。可直接写在已有 `purpose` / `layout` 中，不要求新增整套字段。

发生内容整理时，记录 `原内容或分组 / 处理方式 / 新 section ID 与位置（含 Tab/轮播项） / 保留条件`。合并重复项也要能追溯；没有搬移就不写空表。关键条件与主张同处，不能只在交接文件里保留、却从页面消失。

这样用户能核对业务信息是否仍完整，前端也能延续层级，而不是把已拆开的内容再次等权堆回一段。
